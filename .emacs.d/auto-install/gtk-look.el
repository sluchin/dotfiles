...
;; Copyright 2004, 2006, 2007, 2008, 2009, 2010, 2011, 2012, 2013, 2015 Kevin Ryde

;; Author: Kevin Ryde <user42_kevin@yahoo.com.au>
;; Version: 28
;; Keywords: tools, C, gtk
...
;;
;; There's an autoload cookie for `gtk-lookup-symbol' already if you install
;; via `M-x package-install' or know `update-file-autoloads'.
...
;; Version 26 - emacs20 no \{\} regexp repeat count
;; Version 27 - new email
;; Version 28 - allow for non-breaking spaces in function names
...
;;----------------------------------------------------------------------------
;; compatibility -- with-auto-compression-mode
...
(eval-when-compile
  (put 'gtk-lookup-string-suffix-ci-p 'side-effect-free t))
(defun gtk-lookup-string-suffix-ci-p (suff str)
  "An internal part of gtk-look.el.
...
(eval-when-compile
  (put   'gtk-lookup-assoc-string-ignore-case 'side-effect-free t))
...
" nil t)
                 (let ((type (match-string 2))
                   ...
                   ;; Saw the debian libgtk2.0-doc 2.24.25-3 with an 0xA0
                   ;; non-break space instead of plain space too.
    ...
    (let* ((case-fold-search nil)
      ...
      (if (string-match "[_-]" str)
          ...
          (progn
            ...
            (while (string-match "\\([A-Z][A-Z]+\\)\\([A-Z][a-z]\\)" str)
              (setq str (replace-match "\\1_\\2" t nil str)))
...
;; LocalWords: devhelp libgtk docstring datatype gz usr unparent canonicalized
;; LocalWords: func keyval TreeStore GtkWidget Pango regexps ons el
;; LocalWords: Gtk autoloaded filename wildcards gzip html Lookup gtk gdk
